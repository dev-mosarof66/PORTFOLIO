...
const skills = [
  ...
  { name: "JavaScript", icon: <FaJsSquare className="text-yellow-400" size={40} /> },
  { name: "React", icon: <FaReact className="text-cyan-400" size={40} /> },
  { name: "TypeScript", icon: <SiTypescript className="text-blue-500" size={35} /> },
  { name: "Node JS", icon: <FaNodeJs className="text-green-500" size={40} /> },
  { name: "Express JS", icon: <SiExpress className="text-green-500" size={40} /> },
  { name: "MongoDB", icon: <SiMongodb className="text-green-400" size={40} /> },
  { name: "Git", icon: <FaGitAlt className="text-red-500" size={40} /> },
  { name: "Mongoose", icon: <SiMongoose className="text-pink-400" size={45} /> },
  { name: "Next JS", icon: <SiNextdotjs className="text-white" size={40} /> },
  { name: "Redux", icon: <SiRedux className="text-purple-600" size={40} /> },
  { name: "Firebase", icon: <BiLogoFirebase className="text-amber-600" size={40} /> },
  { name: "Clerk", icon: <SiClerk className="text-white" size={35} /> },
];
...
const Skills = () => {
  ...
  return (
    <div id="Skills" className={`w-full min-h-screen py-20 px-6 ${bgColor}`}>
      <motion.h2
      ...
      <motion.p
        ...
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
        className={`w-full max-w-xl mx-auto text-center py-6 ${textColor}`}
      >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-8 max-w-6xl mx-auto">
        {skills.map((skill, index) => (
          <HoverBorderGradient
            key={index}
            className={`w-full cursor-pointer transition duration-300 delay-75 rounded-sm ${cardBg} ${hoverBg}`}
          >
            <motion.div
            ...
            >
              <div className="flex flex-col items-center gap-1 p-4">
                {skill.icon}
                <p className={`${textColor}`}>{skill.name}</p>
              </div>
            </motion.div>
          </HoverBorderGradient>
        ))}
